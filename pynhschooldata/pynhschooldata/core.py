"""
Core functions wrapping nhschooldata R package via rpy2.
"""
...
def _get_pkg():
    ...
    global _pkg
    if _pkg is None:
        _pkg = importr("nhschooldata")
    return _pkg


def fetch_enr(end_year: int) -> pd.DataFrame:
    """
    Fetch New Hampshire school enrollment data for a single year.
...
def fetch_enr_multi(end_years: list[int]) -> pd.DataFrame:
    """
    Fetch New Hampshire school enrollment data for multiple years.
    ...
    --------
    >>> import pynhschooldata as nh
    >>> df = nh.fetch_enr_multi([2020, 2021, 2022])
    """
    pkg = _get_pkg()
    with localconverter(robjects.default_converter + pandas2ri.converter):
