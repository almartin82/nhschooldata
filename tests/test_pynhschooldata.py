...
def get_test_years():
    ...
    if _available_years is None:
        ...
        _available_years = nh.get_available_years()
...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pynhschooldata as nh
        assert hasattr(nh, 'fetch_enr')
        assert hasattr(nh, 'fetch_enr_multi')
        assert hasattr(nh, 'tidy_enr')
        assert hasattr(nh, 'get_available_years')

    def test_version_exists(self):
        """Package has a version string."""
        import pynhschooldata as nh
        assert hasattr(nh, '__version__')
        assert isinstance(nh.__version__, str)
...
class TestFetchEnr:
    ...
    def test_returns_dataframe(self):
        ...
        df = nh.fetch_enr(years['max_year'])
    ...
    def test_total_enrollment_reasonable(self):
        ...
        if 'is_state' in df.columns and 'grade_level' in df.columns and 'subgroup' in df.columns:
            ...
            if len(total_df) > 0:
                ...
                # New Hampshire should have ~170,000 students
                assert total > 150_000
                assert total < 220_000
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        ...
        df = nh.fetch_enr_multi([years['max_year']])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_contains_requested_year(self):
        ...
        df = nh.fetch_enr_multi([test_year])
    ...
    def test_multi_matches_single(self):
        ...
        import pynhschooldata as nh
        years = get_test_years()
        df_single = nh.fetch_enr(years['max_year'])
        df_multi = nh.fetch_enr_multi([years['max_year']])
