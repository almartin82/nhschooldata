# ==============================================================================
# LIVE Pipeline Tests for nhschooldata
# ==============================================================================
#
...
test_that("New Hampshire DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add New Hampshire DOE URL and verify HTTP 200")
})
...
test_that("Can download New Hampshire enrollment data file", {
  skip_if_offline()
...
test_that("Can parse New Hampshire enrollment file", {
  skip_if_offline()
...
test_that("nhschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- nhschooldata::get_available_years()
  ...
  tryCatch({
    raw <- nhschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- nhschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- nhschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- nhschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- nhschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- nhschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- nhschooldata:::get_cache_path(2024, "enrollment")
