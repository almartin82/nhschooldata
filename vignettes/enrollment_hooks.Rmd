...
## New Hampshire Lost 42,000 Students Since 2002

The Granite State peaked at 207,000 students and has declined ever since. This 20% drop reflects an aging population and declining birth rates.

```{r statewide-data}
enr <- fetch_enr_multi(c(1990, 1995, 2002, 2005, 2010, 2015, 2020, 2024))
...
```{r statewide-chart, fig.cap="New Hampshire statewide enrollment peaked in 2002"}
ggplot(statewide, aes(x = end_year, y = n_students)) +
  geom_line(color = "#2E86AB", linewidth = 1.2) +
  geom_point(color = "#2E86AB", size = 3) +
  geom_vline(xintercept = 2002, linetype = "dashed", color = "#E94F37", alpha = 0.7) +
  annotate("text", x = 2002, y = max(statewide$n_students) * 0.95,
           label = "Peak: 2002", hjust = -0.1, color = "#E94F37") +
  scale_y_continuous(labels = scales::comma, limits = c(0, NA)) +
  labs(
    title = "New Hampshire Public School Enrollment (1990-2024)",
    subtitle = "Lost 42,000 students (-20%) since 2002 peak",
    x = "Year",
    y = "Students"
  )
```
...
White students have declined from 92% to 85% since 2010. Hispanic and multiracial students are the fastest-growing demographic groups.

```{r demographics-data}
enr_demo <- fetch_enr_multi(c(2010, 2015, 2020, 2024))
...
demographics %>%
  ...
  labs(
    title = "Demographic Composition of NH Public Schools (2010-2024)",
    subtitle = "White share declined from 92% to 85%; Hispanic and multiracial growing fastest",
    x = "Year",
...
Coastal communities are holding relatively steady (-4%) while the North Country has lost nearly a third of its students (-29%) since 2015.

```{r regional-data}
seacoast <- c("Portsmouth", "Dover", "Rochester", "Exeter", "Hampton")
north_country <- c("Berlin", "Colebrook", "Lancaster", "Littleton", "Gorham")

enr_regional <- fetch_enr_multi(c(2015, 2018, 2021, 2024))
...
ggplot(regional, aes(x = end_year, y = total, color = region)) +
  ...
  labs(
    ...
    subtitle = "North Country lost 29% of students since 2015; Seacoast only 4%",
    x = "Year",
    y = "Students",
    color = "Region"
  ) +
...
Kindergarten enrollment has dropped 15% since 2015, signaling continued decline ahead. High school grades are declining more slowly for now.

```{r grade-level-data}
enr_grades <- fetch_enr_multi(2015:2024)
...
grades %>%
  ...
  scale_y_continuous(labels = scales::comma) +
  labs(
    title = "New Hampshire Enrollment by Grade Level (2015-2024)",
    subtitle = "Kindergarten down 15%; high school grades declining more slowly",
    x = "Year",
    y = "Students",
    color = "Grade"
  ) +
...
- **Declining enrollment**: Down 42,000 students (20%) since the 2002 peak
- **Urbanization**: Large cities losing ground to suburban districts
- **Diversification**: White share declining as Hispanic and multiracial populations grow
- **Regional divergence**: Coastal areas stable, rural North Country losing students rapidly
- **Kindergarten warning**: 15% fewer kindergartners than 2015 signals continued decline
- **Many tiny districts**: 45 districts under 300 students, managed through SAU sharing
